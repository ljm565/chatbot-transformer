...
class Encoder(nn.Module):
    def __init__(self, config, tokenizer, device):
        ...
        self.encoders = nn.ModuleList([EncoderLayer(self.hidden_dim, self.ffn_dim, self.num_heads, self.bias, self.dropout, self.layernorm_eps) for _ in range(self.enc_num_layers)])
...
class Decoder(nn.Module):
    def __init__(self, config, tokenizer, device):
        ...
        self.emb_layer = TokenEmbeddings(self.vocab_size, self.hidden_dim, self.pad_token_id)
        self.pos_layer = PositionalEmbedding(self.max_len, self.hidden_dim, self.pos_encoding, self.device)
        self.decoders = nn.ModuleList([DecoderLayer(self.hidden_dim, self.ffn_dim, self.num_heads, self.bias, self.dropout, self.layernorm_eps) for _ in range(self.dec_num_layers)])
